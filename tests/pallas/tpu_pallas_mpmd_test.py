...
class MpmdAsyncTest(jtu.JaxTestCase):
  ...
  def test_sc_vmem_shared_to_tc_vmem(self):
    if not jtu.is_libtpu_at_least("0.0.48"):
      self.skipTest("Requires libtpu >= 0.0.48")
    ...
    def vector_subcore_fn(x_ref, out_tc_vmem_ref, tc_sem, sc_vmem_ref, sem):
      pltpu.sync_copy(x_ref, sc_vmem_ref)
      pltpu.async_remote_copy(
          sc_vmem_ref, out_tc_vmem_ref, sem, tc_sem, device_id={"tc": 0}
      ).wait_send()
